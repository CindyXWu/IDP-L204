...
def getColour(): #renamed to getColour to keep consistent reference in main loop		
    ...
    if raw1 > 0.58 or raw2 > 0.58 or raw3 > 0.58: #no need to use lookup table, we already estimated this as threshold		
        led = True #GREEN
        print("Green LED:", raw1, raw2, raw3)			
    else:		
        led = False #RED
        print("Red LED:", raw1, raw2, raw3)		
    return led		
...
def getBlockData(): 		
    ...
    #calculated above		
        if (sensorValueScan[i - 1][2] - alpha) > 0.12:		
            ...
            print("Block discovered at a bearing of ", sensorValueScan[i][3], " with diff = ", (sensorValueScan[i - 1][2] - alpha))		
...
def alternateRoute(desiredxpos, desiredzpos):
    ...
    if twopointturn == False:
        if zfirst == True:
            ...
            while robot.step(TIME_STEP) != -1 and abs(zdiff) > 0.01:	       		
            ...
            while robot.step(TIME_STEP) != -1 and abs(xdiff) > 0.01:	       		
        ...
        while robot.step(TIME_STEP) != -1 and abs(zdiff) > 0.12:
                zdiff = desiredzpos - gps.getValues()[2]
...
def getBearingToPoint(initial_position = [0, 0, -0.4]):		
    current_position = gps.getValues()		
    target_bearing = 0.0		
...
while robot.step(TIME_STEP) != -1:		
    ...
    if scanblocks==True and gotblock == False:
        ...
        if nextTargetIdentified == True:
        	 ...
        	 #bearings = #NEED TO ADD SOMETHING HERE TO CALCULATE BEARINGS FROM GPS LOCATIONS
        ...
        if checkgoround == True:       
            ...
            alternateRoute(GPSOfBlocks[0][0], GPSOfBlocks[0][1])
            motor_left.setVelocity(0)		
            motor_right.setVelocity(0)		
            colour = getColour();		
            if colour == False:		
    ...
    if moveblock == False and blockgreen==True:
        ...
        if altRoute == False:
            print("goin back straightforward")
            returnToStart()
        else:
            print("goin back alt route")
